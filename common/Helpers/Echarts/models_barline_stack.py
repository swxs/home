...
class BarlineStackChart(BaseChart):
    name = model_enums.CHART_TTYPE_BARLINE_STACK

    async def _get_series_index(self, line_field, type_line_data):
        type_line_data.update(yAxis=type_line_data["axis"])
        return await super(BarlineStackChart, self)._get_series_index(line_field, type_line_data)
    ...
    async def to_option(self):
        ...
        await self.set_option_y_sub_axis(format=True, show_line=False)
    ...
    async def to_export_data(self):
        return await super(BarlineStackChart, self).to_export_data()
