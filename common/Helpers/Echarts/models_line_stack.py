...
class LineStackChart(BaseChart):
    name = model_enums.CHART_TTYPE_LINE_STACK

    async def _get_series_index(self, line_field, type_line_data):
        type_line_data.update(yAxis=type_line_data["axis"])
        return await super(LineStackChart, self)._get_series_index(line_field, type_line_data)
    ...
    async def to_export_data(self):
        return await super(LineStackChart, self).to_export_data()
