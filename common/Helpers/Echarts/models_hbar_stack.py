...
class HbarStackChart(BaseChart):
    name = model_enums.CHART_TTYPE_HBAR_STACK

    async def _get_series_index(self, line_field, type_line_data):
        type_line_data.update(dict(xAxis=type_line_data["axis"]))
        return await super(HbarStackChart, self)._get_series_index(line_field, type_line_data)
    ...
    async def to_export_data(self):
        return await super(HbarStackChart, self).to_export_data()
