...
class HbarStackPercentChart(BaseChart):
    name = model_enums.CHART_TTYPE_HBAR_STACK_PERCENT

    async def _get_series_index(self, line_field, type_line_data):
        type_line_data.update(xAxis=type_line_data["axis"])
        return await super(HbarStackPercentChart, self)._get_series_index(line_field, type_line_data)
    ...
    def hook_axis_formatter(self, formatter_kwargs):
        super(HbarStackPercentChart, self).hook_axis_formatter(formatter_kwargs)
        formatter_kwargs.update(dict(ttype="percent"))

    def hook_label_formatter(self, formatter_kwargs):
        super(HbarStackPercentChart, self).hook_label_formatter(formatter_kwargs)
        formatter_kwargs.update(dict(ttype="percent"))
    ...
    async def to_option(self):
        ...
        await self.set_option_data_zoom(multi_color_tag=False)
    ...
    async def to_export_data(self):
        return await super(HbarStackPercentChart, self).to_export_data()
