...
class LineShadowChart(BaseChart):
    name = model_enums.CHART_TTYPE_LINE_SHADOW

    async def _get_series_index(self, line_field, type_line_data):
        type_line_data.update(yAxis=type_line_data["axis"])
        return await super(LineShadowChart, self)._get_series_index(line_field, type_line_data)
    ...
    async def to_option(self):
        ...
        await self.set_option_legend(icon="line")
        await self.set_option_smooth()
    ...
    async def to_export_data(self):
        return await super(LineShadowChart, self).to_export_data()
