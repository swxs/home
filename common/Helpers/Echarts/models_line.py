...
class LineChart(BaseChart):
    name = model_enums.CHART_TTYPE_LINE

    async def _get_series_index(self, line_field, type_line_data):
        type_line_data.update(yAxis=type_line_data["axis"])
        return await super(LineChart, self)._get_series_index(line_field, type_line_data)
    ...
    async def to_export_data(self):
        return await super(LineChart, self).to_export_data()
