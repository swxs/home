...
class BarStackPercentChart(BaseChart):
    name = model_enums.CHART_TTYPE_BAR_STACK_PERCENT

    async def _get_series_index(self, line_field, type_line_data):
        type_line_data.update(yAxis=type_line_data["axis"])
        return await super(BarStackPercentChart, self)._get_series_index(line_field, type_line_data)

    def hook_axis_formatter(self, formatter_kwargs):
        super(BarStackPercentChart, self).hook_axis_formatter(formatter_kwargs)
        formatter_kwargs.update(dict(ttype="percent"))

    def hook_label_formatter(self, formatter_kwargs):
        super(BarStackPercentChart, self).hook_label_formatter(formatter_kwargs)
        formatter_kwargs.update(dict(ttype="percent"))
    ...
    async def to_export_data(self):
        return await super(BarStackPercentChart, self).to_export_data()
