...
TARGET_REDIS_LIST = "redis_list"

SPECIAL_TARGETS = [TARGET_MYSQL, TARGET_POSTGRES, TARGET_SYSLOG, TARGET_SYS_STDOUT, TARGET_SYS_STDERR, TARGET_SYS_STDOUT_ALIAS, TARGET_SYS_STDERR_ALIAS, TARGET_REDIS_LIST]
...
class Logger:
    ...
    def add_target(self, target, *args):
        ...
        if (not target in self.__Logger_targets):
            ...
            elif (target == TARGET_REDIS_LIST):
                if (redis_available == TRUE):
                    # Required parameters: redishost, redisport, redisdb, redispass, redis_list_name
                    try:
                        self.__Logger_redis_connection_pool = redis.ConnectionPool(host=args[0], port=args[1], db=args[2], password=args[3])
                        self.__Logger_redis_connection = redis.Redis(connection_pool=self.__Logger_redis_connection_pool)
                        self.__Logger_redis_list_name = args[4]
                        self.__Logger_targets.append(target)
                    except redis.RedisError, detail:
                        self.error("Redis connection failed: %s" % detail)
                else:
                    self.error("Redis target not added - Pyredis not available")
            else:
    ...
    def __Logger_set_instance_options(self, parser, section, instance):
        """ **(private)** Set the options for a given instance from the parser_utils section """
    ...
    def __Logger_showmessage(self, message, messagesource):
        ...
        for i in range(len(self.__Logger_targets)):
            ...
            if (target == TARGET_MYSQL):
                ...
                self.__Logger_redis_connection.lpush(self.__Logger_redis_list_name, line)
        ...
        if (os.path.exists(self.__FileAppender_filename)) and (self.__FileAppender_rotation != ROTATE_NONE):
            ...
            if self.__FileAppender_rotate(statinfo[8]):
                ...
                try:
                    ...
                    rename(self.__FileAppender_filename, target_file)
        log_dir_name = os.path.dirname(self.__FileAppender_filename)
