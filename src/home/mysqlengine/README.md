...
- 会话通过 `home.web.dependencies.get_db` 注入；Repository 继承 `BaseRepository[Model]`，由 service 在构造函数显式持有（`self.repo = XxxRepository(db)`），写路径事务边界由 service 的 `async with transaction(self.db):` 统一管理（repo 只 flush、不 commit）。
- Repository 与 `home.web.schemas.pagination.PageSchema`、业务 schema 配合做搜索与分页。
