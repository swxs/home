...
assert(
    number_of_buckets and number_of_buckets > 1 and number_of_buckets <= 0x100000000,
    'number_of_buckets: positive integer > 2 and <= 2^29 expected'
)
assert(is_power_of_two(number_of_buckets), 'number_of_buckets: must be power of two (2^n)')

local bucket_size = tonumber(ARGV[2])
assert(bucket_size and bucket_size >= 2 and bucket_size <= 8, 'bucket_size: positive integer >= 2 and <= 8 expected')

local bits_per_fingerprint = tonumber(ARGV[3])
assert(
    bits_per_fingerprint and bits_per_fingerprint >= 7 and bits_per_fingerprint <= 128,
...
assert(total_bytes <= 536870900, 'filter with such params cannot fit redis string')
