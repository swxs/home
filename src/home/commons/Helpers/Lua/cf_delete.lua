--[[
    Delete element from existing Cuckoo Filter.
    Accepted parameters:
        Keys:
            key_name - name of Redis string key with Cuckoo filter
        Arguments:
            elem ... - string with element to delete
        Return value:
            vector of true or false: true if deleted successfully, false if element didn't existed before
--]]
local hdr_size = 6
local bytemask = 0xFF
    ...
    for i = 1, bucket_size do
        ...
        for j = 1, cell_str_len - 1 do
            table.insert(
                str,
                bit.band(
                    bit.bor(bit.lshift(cell_str:byte(j), shift), bit.rshift(cell_str:byte(j + 1), 8 - shift)),
                    bytemask
                )
            )
        end
        ...
        assert(
            cell_str:len() == exp_cell_str_len,
            'incorrect cell_str length: ' .. cell_str:len() .. '!=' .. exp_cell_str_len
        )
...
local function set_row(key, bucket_size, bits, index, row)
    ...
    assert(
        replace_range:len() == math.ceil((math.fmod(offset, 8) + bucket_size * bits) / 8),
        'incorrect replace_range length'
    )
...
local function make_empty(bits)
    return string.rep(string.char(0), math.ceil(bits / 8))
end
...
local function cf_delete(key, number_of_buckets, bucket_size, bits, elem)
...
assert(redis.replicate_commands(), 'assertion failed - script oplog replication is not supported')

local key_name = KEYS[1]
assert(key_name, 'Exactly one key must be specified')

local number_of_buckets, bits_per_fingerprint, bucket_size =
    unpack(redis.call('bitfield', key_name, 'get', 'u32', 0, 'get', 'u8', 0 + 32, 'get', 'u8', 0 + 32 + 8))

-- local res = {}
-- for arg_no, elem in ipairs(ARGV) do
--     table.insert(res, cf_delete(key_name, number_of_buckets, bucket_size, bits_per_fingerprint, elem))
-- end

-- return res

return cf_delete(key_name, number_of_buckets, bucket_size, bits_per_fingerprint, ARGV[1])
