...
- 表级 Repository（继承 ``BaseRepository[Model]``，如 ``UserRepository``、``UserAuthRepository``）：
  负责单表 CRUD 与表内自定义查询。``model`` 以类属性声明，统一构造为 ``Repo(db)``。
- 聚合 Repository（不继承 ``BaseRepository``，如 ``UserSearchRepository``、``UserIdentityRepository``）：
  ...
  仅依赖 session，内部以 ``Repo(db)`` 组合表级 repo。

构造与持有
- repo 由 service 在构造函数里**显式持有**（``self.repo = XxxRepository(db)``），支持可选注入便于单测；
  不再使用 ``SingleWorker`` / ``UnitWorker`` / ``get_repository`` 这类服务定位器写法。

事务边界
- 写路径用 ``async with transaction(self.db):``（见 ``web/dependencies/transaction.py``）统一 commit / rollback；
