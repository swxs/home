...
flowchart LR
    ...
    Service --> Worker[SingleWorker 事务自动提交]
    Worker --> Repo[repository 数据访问]
...
- 不做：直接 `get_single_worker` / 触碰 repository、归属授权判断、计数等副作用、领域规则。
- 端点函数应「薄」：通常为「取参数 -> 调 service -> 包装返回」三步。

### service（业务层）
- 只做：业务规则与领域逻辑、归属/权限授权、副作用编排（如使用计数自增）、解密/加密等策略、决定事务边界。
- 事务边界：service 方法内部使用 `async with get_single_worker(db, Model) as worker:`，沿用 `SingleWorker` 的自动提交/回滚语义。
...
说明：领域纯函数工具（无 HTTP、无 DB）可放在 service 内或独立 `utils/`；不应再用浮在 app 根目录的 `<app>_utils.py` 承载业务逻辑。
...
sequenceDiagram
    ...
    participant W as SingleWorker
    participant R as repository
    C->>A: GET /.../self/{id}
    A->>S: reveal_password(id, user_id)
    S->>W: async with get_single_worker(db, Model)
    W->>R: find_one(id)
    R-->>S: instance
...
要点：
- 事务起止由 service 内的 `async with get_single_worker(...)` 决定，与原 api 内联写法语义一致。
- 一个用例中的多次数据访问应包在同一 worker 上下文中，保证原子性。
...
改造后 `apps/password_lock` 的目标形态：

- `api/password_lock.py`、`api/searcher.py`：仅调用 `PasswordLockService`，函数体收敛为「调 service + success」。
- `services/password_lock_service.py`：`PasswordLockService` 提供 `list/get/create/update/delete/search_self/reveal_password`；`reveal_password` 承接原 `searcher` 的查找 + 归属校验 + `used+1` + 解密（原 `password_lock_utils.get_password` 并入此处）。
- `repositories/password_lock_repository.py`：复用 `BaseRepository` 逻辑，去除 `search_with_name_like` 的重复实现。
- `schemas/`：拆分为 `PasswordLockFilter / Create / Update / Out`。

> 注：当前 password_lock 存在的越权（CRUD 缺 user_id 归属校验）与敏感字段泄漏（响应含明文 `custom.password`）问题，按既定计划在后续安全任务中修复；本规范已为其预留归口（授权属 service、字段过滤属 `XxxOut`）。
