...
async def _index_rows(session) -> list[dict]:
    result = await session.execute(text("SHOW INDEX FROM file_info"))
    return [dict(row) for row in result.mappings().all()]


async def _ensure_target_user(session, user_id: str) -> bytes:
    try:
...
async def _ensure_nullable_user_id(session) -> None:
...
async def _load_rows(session, *, has_user_id: bool):
...
async def _finalize_schema(session) -> None:
    ...
    indexes = await _index_rows(session)
    ...
    for name, rows in grouped.items():
        ...
        if name != "PRIMARY" and rows[0]["Non_unique"] == 0 and columns == ["file_id"]:
            await session.execute(text(f"ALTER TABLE file_info DROP INDEX `{name}`"))
...
async def migrate(
    *,
    ...
    async with open_session() as session:
        raw_user_id = await _ensure_target_user(session, user_id)
        has_user_id = await _column_exists(session, "user_id")
        rows = await _load_rows(session, has_user_id=has_user_id)

        if dry_run:
            print("DRY RUN: 不修改数据库或 OSS")
        else:
            await _ensure_nullable_user_id(session)
